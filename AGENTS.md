...
Avalonia 12 上で動く **Velopack 自動更新ダイアログ** の再利用可能 NuGet パッケージ。利用者向けドキュメント（インストール・使い方・オプション一覧）は [README.md](README.md) を正とする。本ファイルは開発者・Codex（ChatGPT）などのコーディングエージェント向けに、リポジトリ構造とビルド・設計上の非自明な約束事を記す。
